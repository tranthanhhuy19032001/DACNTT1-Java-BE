<%@include file="/common/taglib.jsp"%>
<c:url var="APIurl" value="/api-admin-new"/>
<c:url var ="NewURL" value="/admin-new"/>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
	<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
	<html>

	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
		<title>Danh sách bài viết</title>
	</head>

	<body>
		<div class="main-content">
		<form action="<c:url value='/admin-new'/>" id="formSubmit" method="get">
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							<a href="<c:url value="/admin-home"/>">
								<li>
									<i class="ace-icon fa fa-home home-icon"></i>
									Trang chủ</a>
								</li>
							</a>
						</ul>
						<!-- /.breadcrumb -->
					</div>
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								<c:if test="${not empty messageResponse}">
									<div class="alert alert-${alert}">
  										${messageResponse}
									</div>
								</c:if>
								<div class="widget-box table-filter">
									<div class="table-btn-controls">
										<div class="pull-right tableTools-container">
											<div class="dt-buttons btn-overlap btn-group">
												<a flag="info"
												   class="dt-button buttons-colvis btn btn-white btn-primary btn-bold" data-toggle="tooltip"
												   title='Thêm bài viết' href='<c:url value="/admin-new?type=edit"/>'>
															<span>
																<i class="fa fa-plus-circle bigger-110 purple"></i>
															</span>
												</a>
												<button id="btnDelete" type="button"
														class="dt-button buttons-html5 btn btn-white btn-primary btn-bold" data-toggle="tooltip" title='Xóa bài viết'>
																<span>
																	<i class="fa fa-trash-o bigger-110 pink"></i>
																</span>
												</button>
											</div>
										</div>
									</div>
								</div>
								<div class="row">
									<div class="col-xs-12">
										<div class="table-responsive">
											<table class="table table-bordered">
												<thead>
													<tr>
														<th><input type="checkbox" id="all-item-checkbox-btn"></th>
														<th>Tên bài viết</th>
														<th>Mô tả ngắn</th>
														<th>Thao tác</th>
													</tr>
												</thead>
												<tbody>
													<c:forEach var="item" items="${model.listResult}">
														<tr>
															<td><input class="item-checkbox-btn" type="checkbox" value="${item.id}" name="newsIds[]"></td>
															<td>${item.title}</td>
															<td>${item.shortDescription}</td>
															<td>
																<c:url var="editURL" value="/admin-new">
																	<c:param name="type" value="edit"/>
																	<c:param name="id" value="${item.id}"/>
																</c:url>
																<a class="btn btn-sm btn-primary btn-edit" data-toggle="tooltip"
																   title="Cập nhật bài viết" href='${editURL}'><i class="fa fa-pencil-square-o" aria-hidden="true"></i>
																</a>

																<a class="detail-show-btn btn btn-sm btn-danger btn-edit" href="" data-id="${item.id}"
																   data-bs-toggle="modal" data-bs-target="#delete-post-modal">
																	Xóa
																</a>
															</td>
														</tr>
													</c:forEach>
												</tbody>
											</table>

											<c:if test="${not empty model.listResult}">
												<ul class="pagination" id="pagination"></ul>
											</c:if>
											<c:if test="${empty model.listResult}">
												<p class="text-center">Chưa có bài viết nào!</p>
											</c:if>


											<input type="hidden" value="" id="page" name="page"/>
											<input type="hidden" value="" id="maxPageItem" name="maxPageItem"/>
											<input type="hidden" value="" id="sortName" name="sortName"/>
											<input type="hidden" value="" id="sortBy" name="sortBy"/>
											<input type="hidden" value="" id="type" name="type"/>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
		</form>
		</div>

		<!-- Modal -->
		<div class="modal fade" id="delete-post-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalLabel">Xóa tin này?</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
					<div class="modal-body">
						Bạn chắc chắn xóa bài đăng này?
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
						<button id="btn-delete-post" type="button" class="btn btn-danger">Xóa</button>
					</div>
				</div>
			</div>
		</div>

		<!-- /.main-content -->
		<script>
			var totalPages = ${model.totalPage};
			var currentPage = ${model.page};
			var limit = 2;
			$(function () {
				window.pagObj = $('#pagination').twbsPagination({
					totalPages: totalPages,
					visiblePages: 10,
					startPage: currentPage,
					onPageClick: function (event, page) {
						if (currentPage != page) {
							$('#maxPageItem').val(limit);
							$('#page').val(page);
							$('#sortName').val('title');
							$('#sortBy').val('desc');
							$('#type').val('list');
							$('#formSubmit').submit();
						}
					}
				});
			});

			var data = {};
			var ids = []
			var deleteId;

			$("#delete-post-modal").on('show.bs.modal', (event) => {
				const btnDelete = event.relatedTarget;
				// Extract info from data-id attributes
				deleteId = btnDelete.getAttribute('data-id');
				ids.push(deleteId);
				console.log(ids);
			})

			//when all-item-checkbox-btn changed
			$("#all-item-checkbox-btn").change(()=>{
				const isCheckedAll = $("#all-item-checkbox-btn").prop('checked')
				$('input[name="newsIds[]"]').prop('checked', isCheckedAll)
				renderCheckALLSubmitBtn()
			})

			//when item-checkbox-btn changed
			$(".item-checkbox-btn").change(function () {
				const isCheckedAll = $('input[name="newsIds[]"]:checked').length === $(".item-checkbox-btn").length
				$("#all-item-checkbox-btn").prop('checked', isCheckedAll)
				renderCheckALLSubmitBtn()
			})

			//Re-render check all submit btn
			function renderCheckALLSubmitBtn() {
				const isCheckedCount = $('input[name="newsIds[]"]:checked').length
				if (isCheckedCount > 0) {
					$("#btnDelete").attr('disabled', false)
				} else {
					$("#btnDelete").attr('disabled', true)
				}
			}

			$("#btn-delete-post").click(function() {
				// ids = $('tbody input[type=checkbox]:checked').map(function () {
				// 	return $(this).val();
				// }).get();
				data['ids'] = ids;
				// console.log(data);
				deleteNew(data);
			});

			$("#btnDelete").click(function() {
				ids = $('tbody input[type=checkbox]:checked').map(function () {
					return $(this).val();
				}).get();
				data['ids'] = ids;
				// console.log(data);
				deleteNew(data);
			});

			function deleteNew(data) {
				$.ajax({
					url: '${APIurl}',
					type: 'DELETE',
					contentType: 'application/json',
					data: JSON.stringify(data),
					success: function (result) {
						window.location.href = "${NewURL}?type=list&page=1&maxPageItem=2&sortName=title&sortBy=desc&message=delete_success";
					},
					error: function (error) {
						window.location.href = "${NewURL}?type=list&maxPageItem=2&page=1&message=error_system";
					}
				});
			}
		</script>
	</body>

	</html>